...
const BookingPage = () => {
  const { id, roomType } = useParams(); // e.g., roomType = "room0"
  ...
  // Extract room index from roomType (e.g., "room0" -> 0)
  ...
  const handleBooking = async (event) => {
    ...
    const bookingData = {
      hotelId: id,
      roomType: roomName,
      roomIndex, // Include for backend reference
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => navigate("/payment", { state: { bookingData, amount: totalPrice } }), 2000);
      } else {
        setModal({
          isOpen: true,
    ...
    <div className="min-h-screen bg-gray-100 py-8">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={() => navigate(`/hotels/${id}`)} // Updated to match route
